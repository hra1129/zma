...
bool CZMA_PARSE::check_location_hl( int index ) {
	if( (int)words.size() >= (index + 3) && words[index] == "[" && words[(int)(index+1)] == "HL" && words[(int)(index+2)] == "]" ) {
		return true;
	}
	return false;
}
...
int CZMA_PARSE::check_location_ix( int index ) {
	...
	if( words[index] == "[" && words[(int)(index + 1)] == "IX" && words[(int)(index + 2)] == "]" ) {
		return index + 3;
	}
	if( words[index] != "[" || words[(int)(index + 1)] != "IX" || (words[(int)(index + 2)] != "+" && words[(int)(index + 2)] != "-") ) {
		return 0;
	}
...
int CZMA_PARSE::check_location_iy( int index ) {
	...
	if( words[index] == "[" && words[(int)(index + 1)] == "IY" && words[(int)(index + 2)] == "]" ) {
		return index + 3;
	}
	if( words[index] != "[" || words[(int)(index + 1)] != "IY" || (words[(int)(index + 2)] != "+" && words[(int)(index + 2)] != "-") ) {
		return false;
	}
	for( index += 2; (size_t)index < words.size() && words[index] != "]"; index++ );
	if( (size_t)index >= words.size() ) {
...
int CZMA_PARSE::relative_address( CZMA_INFORMATION &info, int index ) {
	...
	if( this->code_address == -1 ){
		put_error( CZMA_ERROR::get( CZMA_ERROR_CODE::ADDRESS_DETERMINED ), info.all_error_enable, info.error_disable );
		info.error_disable = true;
		return -9999;
	}
	index = this->expression( info, index, imm );
	if( index == 0 ) {
		put_error( CZMA_ERROR::get( CZMA_ERROR_CODE::ILLEGAL_EXPRESSION ), info.all_error_enable, info.error_disable );
		info.error_disable = true;
		return -9999;
	}
...
// --------------------------------------------------------------------
void CZMA_PARSE::put_error( std::string message, bool is_all_error, bool error_disable ) {
	...
	if( message != last_error ){
		log.write_error_message( p_file_name, this->line_no, message, is_all_error, error_disable );
		number_of_error++;
...
// --------------------------------------------------------------------
void CZMA_PARSE::put_structure_error( std::string message, bool is_all_error, bool error_disable ) {
	std::stringstream ss;
	std::string s;
	...
	log.write_error_message( this->p_file_name, this->line_no, s, is_all_error, error_disable );
	number_of_error++;
}
