...
bool CZMA_PARSE_RRC::process( CZMA_INFORMATION& info, CZMA_PARSE* p_last_line ) {
	...
	if( this->opecode_source8( info, 0xCB, 0x08 ) ) {
		//	log
		if( !this->is_analyze_phase ) {
			log.write_line_infomation( this->line_no, this->code_address, this->file_address, get_line() );
			if( data.size() == 2 ) {
				if( this->data[1] == 0x0E ) {
					log.write_cycle_information( 17, 8 );			//	RRC [HL]
				}
				else {
					log.write_cycle_information( 10, 2 );			//	RRC r
				}
			}
			else {
				log.write_cycle_information( 25, 10 );			//	RRC [IX+d]
			}
